function mailCount(emailData) {
  ...
  var first;
  var last;
  emailData.split('\n').join(' ').split('##||##').forEach(function (email) {
    emailArray.push(email.split('#/#'));
  });

  emailArray.forEach(function (email) {
    datesArray.push(email[2]);
  });
  datesArray.filter(function (date) {
    var d = [];
    d = date.split(' ');
    emailDates.push(d[2]);
  });
  emailDates.sort(function (date, date2) {
    var a = new Date(date);
  ...
  last = String(new Date(emailDates[emailDates.length - 1])).split(' ').slice(0,4).join(' ');
  first = String(new Date(emailDates[0])).split(' ').slice(0,4).join(' ');
